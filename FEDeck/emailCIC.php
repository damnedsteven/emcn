...
//---------------------------------------------------------------------------------------------------------------------------------------------------
	...
	$query = "  IF OBJECT_ID('dbo.CICPLOEmailed', 'U') IS NULL
					BEGIN
						CREATE TABLE CICPLOEmailed
						(
						PLO VARCHAR(32),
						EmailedTime DATETIME,
						);
						CREATE INDEX CICPLOEmailedIndex
						ON CICPLOEmailed (PLO);
					END
	...
	if (!mssql_num_rows($data)) {
		...
		$PLOMailedArray = array();
		while ($row = mssql_fetch_assoc($data)){
			array_push($PLOMailedArray,trim($row['PLO']));//获取 PLO at 112DB
		}
		$PLOMailed = "'".implode("','", $PLOMailedArray)."'";
	}
