...
//---------------------------------------------------------------------------------------------------------------------------------------------------
	...
	// Connect to Solar DB	
	$dbc = mssql_connect(DB_HOST_SOLAR, DB_USER_SOLAR, DB_PASSWORD_SOLAR) or die("connect db error"); 
	mssql_select_db(DB_NAME_SOLAR,$dbc) or die('can not open db table');
	...
	if (!empty($entry)) {		
		...
		while ($row = mssql_fetch_assoc($data)){	
			...
			echo '<tr>';
				...
				$XPATH[trim($row['serialNumber'])]['MAC'] = $xml[trim($row['serialNumber'])]->xpath('//device[@class="networkDevice"]/property[@name="MAC_ADDR"]/@value');
				...
				echo '<td>'.$SN[trim($row['serialNumber'])]['MAC'].'</td>';
				//
