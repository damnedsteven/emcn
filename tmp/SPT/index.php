	...
	mssql_query("SET ANSI_WARNINGS ON", $dbc);
		...
		$query = "  SELECT 
					...
					FROM
						...
						LEFT JOIN
						--(SELECT DISTINCT [Complexity Groups], [Sales Order ID], [Sales Order Line Item ID] FROM [104].VOM.dbo.DailyPROExtraction WHERE [Plant Code] = 'BF02' AND [Higher Level Item Flag] = 'Y' AND [Base Quantity] <> '0') T2
						[104].VOM.dbo.DailyPROExtraction T2
						ON PCTMaster.SO collate Chinese_PRC_CI_AS=T2.[Sales Order ID] AND PCTMaster.Line collate Chinese_PRC_CI_AS=T2.[Sales Order Line Item ID]
					WHERE
						-- Family LIKE 'TSG%'
						-- AND
